...
public class ProductsController : Controller
{
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [UserHasPermission(PermissionId.ProductsRead)]
    public async Task<ActionResult<IsProductCodeUnique.Response>> IsCodeUnique(
        [FromQuery] IsProductCodeUnique.Request request)
    {
    ...
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [UserHasPermission(PermissionId.ProductsRead)]
    public async Task<ActionResult<IsProductNameUnique.Response>> IsNameUnique(
        [FromQuery] IsProductNameUnique.Request request)
    {
        var response = await _mediator.Send(request);
        return response.ToActionResult();
    }
